...
TissueSamples <- read.csv(here("TissueDataset_10.20.25.csv"), fileEncoding="latin1")

#remove any blank non-bats or non-rodents
...
#load georegion
geo=read.csv(here("georegion.csv"))
...
# Fig. 3A was constructed in arcGIS using the following exported data from R
#write.csv(percountry, "country data for GIS.csv", row.names = FALSE)
...
#write.csv(museums, "institutions.csv", row.names = FALSE) 

##this file^ was then manually georeferenced and used to create the arcGIS map (Fig. 3B)
...
#write.csv(mus, "Supplementary Data S1.csv", row.names = FALSE)
...
#frequency of years per sample
tis_years <- as.data.frame.table(table(TissueSamples$year))
tis_years$Var1 <- as.numeric(as.character(tis_years$Var1))

##read in dates of ALL samples, not just filtered dataset
dat_years <- read.csv(here("dat_years.csv"))

###Generalized Additive Models to see slope of tissue collection over time
...
#identifying periods of increase and decrease
preds$signif_col = NA
...
#slope of tissue collection over time
fig2a <- ggplot() + 
...
#create separate variables for different categories of terms
TissueSamples = TissueSamples %>%
...
alluv <- TissueSamples %>% dplyr::select(etohterms, freezeterms, 
                                         heart, kidney, liver, lung, muscle, 
                                         spleen)

alluv <- alluv %>% dplyr::mutate(Total = dplyr::select(., etohterms:spleen) %>% rowSums(na.rm = TRUE))
alluv <- alluv[-which(alluv$Total < 2),] #remove observations where no terms or one term appears
alluv[which(alluv$etohterms == 1),1] <- "Ethanol"
alluv[which(alluv$freezeterms == 1),2] <- "Frozen"
#alluv[which(alluv$bufferterms == 1),3] <- "Buffer" #negligible
alluv[which(alluv$heart == 1),3] <- "Heart"
alluv[which(alluv$kidney == 1),4] <- "Kidney"
alluv[which(alluv$liver == 1),5] <- "Liver"
alluv[which(alluv$lung == 1),6] <- "Lung"
alluv[which(alluv$muscle == 1),7] <- "Muscle"
alluv[which(alluv$spleen == 1),8] <- "Spleen"
#alluv[which(alluv$colon == 1),10] <- "Colon" #negligible

freq <- data.frame(matrix(ncol = 3, nrow = 0))
colnames(freq) <- c("Preservation Method","Tissue","Frequency")

##frequency of combinations of tissue/preservation method
for (i in 1:2) {
  for (j in 3:8) {
    x <- alluv %>% group_by(.dots = names(.)[c(i,j)]) %>% dplyr::summarise(n = n())
    colnames(x) <- c("Preservation Method","Tissue","Frequency")
    freq <- rbind(freq, as.matrix(x[4,]))
  }
}

freq$Frequency <- as.numeric(freq$Frequency)
freq$`Preservation Method` <- as.factor(freq$`Preservation Method`)
freq$Tissue <- as.factor(freq$Tissue)

alluvialfig1 <- ggplot(as.data.frame(freq),
                       aes(y = Frequency, axis2 = `Preservation Method`, axis1 = Tissue)) +
  geom_alluvium(aes(fill = Tissue), width = 1/12) +
  geom_stratum(width = 1/12, fill = "black", color = "grey") +
  geom_label(stat = "stratum", aes(label = after_stat(stratum))) +
  scale_x_discrete(limits = c("Preservation Method", "Tissue"), expand = c(.05, .05)) +
  scale_fill_brewer(type = "qual", palette = "Set1") + theme_classic() +
  scale_y_continuous(labels = label_comma()) + theme(legend.position = "none")

#ggsave("alluvialfig1.png", alluvialfig1)

###tissue types over time

tis_type_years <- TissueSamples %>% group_by(year) %>% 
  dplyr::summarise(Unspecified=n(), `Blood/serum`=sum(bloodterms), Tissue=sum(tissueterms), Biopsy=sum(biopsyterms),
                   Swab=sum(swabterms), Ethanol=sum(etohterms), Frozen=sum(freezeterms),
                   Buffer=sum(bufferterms), heart=sum(heart), kidney=sum(kidney), liver=sum(liver),
                   lung=sum(lung), muscle=sum(muscle), spleen=sum(spleen), colon=sum(colon))

colors <- c("Unspecified" = "grey90", "Frozen" = "light blue", "Ethanol" = "blue",
            "Blood/serum" = "red", "Swab" = "green", "Buffer" = "black",
            "Biopsy" = "orange")

fig2b <- ggplot() +
  geom_bar(mapping = aes(x = year, y = Unspecified, fill="Unspecified"), stat = 'identity',
           data = tis_type_years %>% filter(year > 1900)) +
  geom_bar(mapping = aes(x = year, y = Frozen, fill="Frozen"), stat = 'identity',
           data = tis_type_years %>% filter(year > 1900)) +
  geom_bar(mapping = aes(x = year, y = Ethanol, fill="Ethanol"), stat = 'identity', alpha=0.8,
           data = tis_type_years %>% filter(year > 1900)) +
  geom_bar(mapping = aes(x = year, y = `Blood/serum`, fill="Blood/serum"), stat = 'identity', alpha=0.7,
           data = tis_type_years %>% filter(year > 1900)) +
  geom_bar(mapping = aes(x = year, y = Swab, fill="Swab"), stat = 'identity', alpha=0.9,
           data = tis_type_years %>% filter(year > 1900)) +
  geom_bar(mapping = aes(x = year, y = Buffer, fill="Buffer"), stat = 'identity', alpha=1,
           data = tis_type_years %>% filter(year > 1900)) +
  geom_bar(mapping = aes(x = year, y = Biopsy, fill="Biopsy"), stat = 'identity', alpha=1,
           data = tis_type_years %>% filter(year > 1900)) +
  labs(x = "Year",
       y = "Count",
       fill = "Preservation Method") +
  scale_fill_manual(values = colors) +
  theme_classic() + theme(legend.position = c(0.2,0.73))

fig2 <- ggarrange(fig2a,fig2b,ncol=1,nrow=2,#widths=c(4,4),heights=c(22,1),
                  labels=c("A","B"),
                  #label.x=c(0,-0.1), label.y=0.001,
                  font.label=list(face="plain",size=12))

ggsave("Fig 2.jpeg", fig2, height=9, width=6)
...
## load taxonomy and phylogeny (Upham et al. 2019)
tree=read.nexus(here('MamPhy_fullPosterior_BDvr_Completed_5911sp_topoCons_NDexp_MCC_v2_target.tre'))
taxonomy=read.csv(here("taxonomy_mamPhy_5911species.csv"))
...
########manual relabeling using Mammal Diversity Database
###October 5, 2025, using MDD v2.3, released September 1, 2025
