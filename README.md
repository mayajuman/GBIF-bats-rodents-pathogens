...
# Overview and files
This study investigates the utility of biodiversity databases for locating museum samples for pathogen detection work. Here, we filtered all bat and rodent records in GBIF to locate tissue samples for pathobiology work and assess spatial, temporal, and taxonomic trends in these samples. The unfiltered rodent and bat GBIF data are stored <a href="https://zenodo.org/records/18663923" target="_blank">here</a>, along with the downloaded filtered tissue dataset. The (large) unfiltered files can be read into the "cleaning.R" script to generate the filtered dataset. To associate latitude, longitude, and region with countries/museums for visualization and statistical models, the "LatLong.csv", "georegion.csv", and "institutions.csv" files here can be read into the scripts. We also include the tree files from <a href="https://journals.plos.org/plosbiology/article?id=10.1371/journal.pbio.3000494" target="_blank">Upham et al. 2019</a> which we used for our analyses of taxonomic bias.
